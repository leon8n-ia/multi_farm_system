"""ReactNextjsFarm — produces React/Next.js prompt packs via competing producer agents."""
...
_DEFAULT_SELLER_STRATEGY: dict = {
    ...
    "target_audience": "developers using Cursor and Claude",
...
class ReactNextjsFarm(BaseFarm):
    """Farm that produces React/Next.js prompt packs via competing producer agents."""

    def __init__(
        self,
        id: str,
        name: str = "React/Next.js Farm",
        capital: float = 1000.0,
        credits: float = 500.0,
    ) -> None:
        super().__init__(id, name, FarmType.REACT_NEXTJS, capital, credits)
    ...
    def _init_producer_agents(self) -> None:
        ...
        # Agent 1: Cursor prompts focus
        agent1 = Agent(
            id=f"{self.id}-prompt-001",
        ...
        # Agent 2: Boilerplate focus
        agent2 = Agent(
            id=f"{self.id}-prompt-002",
            credits=float(INITIAL_CREDITS),
            strategy={
                "niche_focus": "react_nextjs",
                "product_type": "prompt_pack",
                "product_variant": "boilerplate_templates",
                "price_target": 45.0,
        ...
        # Agent 3: Hooks & patterns focus
        agent3 = Agent(
            id=f"{self.id}-prompt-003",
            credits=float(INITIAL_CREDITS),
            strategy={
                "niche_focus": "react_nextjs",
                "product_type": "prompt_pack",
                "product_variant": "hooks_patterns",
                "price_target": 35.0,
        ...
        self.producer_agents = [
            PromptPackAgent(agent1),
            PromptPackAgent(agent2),
            PromptPackAgent(agent3),
        ]
    ...
    def run_production(self) -> None:
        ...
        for pa in self.producer_agents:
            variant = pa.agent.strategy.get("product_variant", "cursor_prompts")
    ...
    def run_sales(self) -> None:
        ...
        for product in self.output_buffer:
            title = product.get("title", "React/Next.js Prompt Pack")
            description = product.get("description", "Premium prompts for React developers.")
            price = product.get("price", self.seller_agent.strategy.get("base_price", 39.0))
    ...
    def reproduce_winners(self) -> None:
        ...
        self.producer_agents.append(PromptPackAgent(child))
