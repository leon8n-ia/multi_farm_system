...
from farms.monetized_content.producer_agent import NICHES, ProducerAgent
...
_DEFAULT_STRATEGY: dict = {
    ...
    "niche_focus": "ai_automation",
    "output_format": "markdown",
}


class MonetizedContentFarm(BaseFarm):
    def __init__(
        self,
        id: str,
        name: str,
        ...
        self.niches: list[str] = niches or ["ai_automation", "python_tutorials", "data_science_tips"]
        ...
        self.revenue_bridge = RevenueBridgeRouter([
            ...
            GumroadRevenueBridge(product_id=GUMROAD_PRODUCT_IDS["monetized_content"]),
    ...
    def run_sales(self) -> None:
        ...
        output_fmt = self.seller_agent.strategy.get("output_format", "markdown")
        ...
        for item in self.output_buffer:
            ...
            listing["title"] = f"{niche_label} Guide [{output_fmt.upper()}]"
            listing["description"] = (
                f"In-depth {niche_label.lower()} guide — "
                f"practical examples, step-by-step tutorials, and actionable insights. "
            ...
            if random.random() < SALE_PROBABILITY:
                ...
                self.revenue_bridge.publish_product(
                    title=listing.get("title", "Premium Article"),
                    description=listing.get("description", ""),
                    price_usd=price,
                )
                # Upload to Google Drive
                file_name = f"content_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
