...
_NICHES = ["crypto_analysis", "saas_metrics", "ecommerce_performance"]

_DEFAULT_STRATEGY: dict = {
    "primary_channel": "fiverr",
    "pricing_model": "fixed",
    "base_price": 29.0,
    "discount_threshold": 3,
    "discount_rate": 0.15,
    "listing_quality": "high",
    "target_audience": "developers and business analysts",
    "bundle_strategy": False,
    "niche_focus": "crypto_analysis",
    "output_format": "pdf",
}


class AutoReportsFarm(BaseFarm):
    def __init__(
        self,
        id: str,
        name: str,
    ...
    def run_production(self) -> None:
        for pa in self.producer_agents:
            result = pa.execute_task(self._current_niche())
            if result.success:
    ...
    def run_sales(self) -> None:
        ...
        output_fmt = self.seller_agent.strategy.get("output_format", "pdf")
        ...
        for item in self.output_buffer:
            ...
            listing["title"] = f"{niche_label} Financial Report [{output_fmt.upper()}]"
            listing["description"] = (
                f"Comprehensive {niche_label.lower()} financial analysis — "
                f"revenue growth, margins, cash flow, and risk assessment. "
            ...
            if random.random() < SALE_PROBABILITY:
                ...
                self.revenue_bridge.publish_product(
                    title=listing.get("title", "Financial Report"),
