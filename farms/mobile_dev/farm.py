"""MobileDevFarm — produces mobile development starter kits via competing producer agents."""
...
class MobileDevFarm(BaseFarm):
    """Farm that produces mobile development starter kits via competing producer agents."""

    def __init__(
        self,
        id: str,
        name: str = "Mobile Dev Farm",
        capital: float = 1000.0,
        credits: float = 500.0,
    ) -> None:
        super().__init__(id, name, FarmType.MOBILE_DEV, capital, credits)
        self.economy = EconomyEngine()
        self.seller_agent = MobileDevSellerAgent(farm_id=id)
        self.product_type = "starter_kit"
        self.revenue_bridge = RevenueBridgeRouter([
            LemonSqueezyRevenueBridge(),
            GumroadRevenueBridge(),
        ], farm_type="mobile_dev")

        # Initialize competing producer agents
    ...
    def _init_producer_agents(self) -> None:
        ...
        # Agent 1: React Native AI focus
        agent1 = Agent(
            id=f"{self.id}-reactnative-001",
        ...
        # Agent 2: Flutter starter kit focus
        agent2 = Agent(
            id=f"{self.id}-flutter-002",
            credits=float(INITIAL_CREDITS),
            strategy={
                "niche_focus": "mobile_dev",
                "product_type": "starter_kit",
        ...
        # Agent 3: Mobile prompts focus
        agent3 = Agent(
            id=f"{self.id}-prompts-003",
            credits=float(INITIAL_CREDITS),
            strategy={
                "niche_focus": "mobile_dev",
        ...
        self.producer_agents = [
            ReactNativeAgent(agent1),
            FlutterAgent(agent2),
            MobilePromptsAgent(agent3),
        ]
    ...
    def run_production(self) -> None:
        ...
        for pa in self.producer_agents:
            variant = pa.agent.strategy.get("product_variant", "react_native_ai")
    ...
    def run_sales(self) -> None:
        ...
        for product in self.output_buffer:
            title = product.get("title", "Mobile Dev Starter Kit")
            description = product.get("description", "Essential components for mobile developers.")
            price = product.get("price", self.seller_agent.strategy.get("base_price", 34.0))
            ...
            if random.random() < SALE_PROBABILITY:
                ...
                file_name = f"starterkit_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def reproduce_winners(self) -> None:
        ...
        if isinstance(parent_pa, FlutterAgent):
            new_agent = FlutterAgent(child)
        elif isinstance(parent_pa, MobilePromptsAgent):
            new_agent = MobilePromptsAgent(child)
        else:
            new_agent = ReactNativeAgent(child)
