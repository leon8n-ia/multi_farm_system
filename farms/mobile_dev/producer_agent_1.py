"""ReactNativeAgent — generates React Native AI starter kits for mobile developers."""
...
DEFAULT_STRATEGY: dict[str, Any] = {
    ...
    "product_variant": "react_native_ai",
    "price_target": 34.0,
...
SYSTEM_PROMPT = """You are a senior mobile developer creating premium React Native starter kits for mobile developers using AI tools.

Your task is to generate high-quality, production-ready starter kit components that help developers:
- Build React Native apps with AI integrations (OpenAI, Claude, local LLMs)
- Implement best practices for mobile AI applications
- Ship faster with ready-to-use components and utilities
...
Focus on practical, production-ready code. Include real-world examples and common use cases."""


class ReactNativeAgent:
    """Producer agent specialized in React Native AI starter kits."""
    ...
    def execute_task(self, variant: str | None = None) -> TaskResult:
        """Generate a starter kit product using Claude API."""
        start = time.perf_counter()
        strategy = self.agent.strategy
        variant = variant or strategy.get("product_variant", "react_native_ai")
        ...
        try:
            ...
            logger.warning("[ReactNativeAgent] Claude API failed: %s — using fallback", exc)
            return self._fallback_generation(variant, start)
    ...
    def _generate_with_claude(self, variant: str, api_key: str) -> dict:
        ...
        audience = strategy.get("audience", "mobile developers using AI tools")
...
- Target items: {items_per_pack} components/utilities (generate at least 20 high-quality examples)
- Target price: ${price_target}
- Niche: Mobile development with AI integrations
...
Each item in content should have: category, name, description, code_snippet, usage_example, tips."""
    ...
    def _fallback_generation(self, variant: str, start_time: float) -> TaskResult:
        ...
        self.last_output = {
            "title": f"React Native AI {variant.replace('_', ' ').title()} Starter Kit",
            "description": f"Production-ready {variant.replace('_', ' ')} components for mobile developers building AI-powered apps.",
            "content": [
                {
                    "category": "AI Integration",
                    "name": "useClaudeChat",
                    "description": "React hook for Claude API chat completions",
                    "code_snippet": "const { messages, sendMessage, isLoading } = useClaudeChat({ apiKey });",
                    "usage_example": "Wrap your chat component and call sendMessage('Hello')",
                    "tips": "Use streaming for better UX on mobile devices",
                },
                {
                    "category": "AI Integration",
                    "name": "AIResponseCard",
                    "description": "Styled card component for AI responses with markdown support",
                    "code_snippet": "<AIResponseCard response={aiResponse} showCopyButton />",
                    "usage_example": "Display AI responses with proper formatting and copy functionality",
                    "tips": "Enable syntax highlighting for code blocks",
                },
                {
                    "category": "Utilities",
                    "name": "tokenCounter",
                    "description": "Utility to estimate token count before API calls",
                    "code_snippet": "const tokens = tokenCounter.estimate(prompt);",
                    "usage_example": "Check token count before sending to avoid API limits",
                    "tips": "Cache results for repeated prompts",
                },
            ],
            "price": price,
        }
